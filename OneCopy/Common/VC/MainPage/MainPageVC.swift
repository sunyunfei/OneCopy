    ...
    func getDatas() -> Void {
        ...
        HttpManager.homeGet { dataArray in
            ...
            for dic:Dictionary<String,AnyObject> in dataArray{
                ...
                //转变为model
                let model : MainPageCardModel = MainPageCardModel.init()
                model.volNum = dic["volNum"] as! Int;
                model.writerName = dic["writerName"] as! NSString?
                model.content = dic["content"] as! NSString?
                model.writeTime = dic["writeTime"] as! NSString?
                model.zanNum = dic["zanNum"] as! Int
                model.picUrl = dic["picUrl"] as! NSString?
                model.layout()
                model.isZan = (dic["isZan"] != nil)
